...
func CreateTables() error {
    ...
    for _, table := range tables {
        _, err := DB.Exec(table) // Use the global DB from this package
        if err != nil {
...
// Utility function (can also be in a separate `utils` package if many)
func IsDuplicateKeyError(err error) bool {
    if err == nil {
        return false
    }
    // Specific SQLite error message for unique constraint violation
