...
export class PostManager {
  ...
  setupPostEventListeners() {
    document
      ...
      ?.addEventListener("submit", (e) => this.handlePostCreate(e));
      ...
      .map(
        (post) => `
            <div class="post" data-id="${post.id}">
                ...
                <div class="post-meta">
                    <span>Posted by ${post.author || "Unknown"} in ${
  ...
  async showCommentPopup(postId) {
    try {
      ...
      document.getElementById("popup-post-title").textContent =
        post.title || "Post";
  ...
  async handleCommentCreate(e) {
    const container = document.getElementById("popup-comments-container");
    e.preventDefault();
    const token = this.getCookie("session_id");
    try {
      const response = await fetch(`/api/auto?with=${token}`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        container.onscroll = this.throttle(() => {
  ...
  async loadComments(postId, containerId = "popup-comments-container") {
    try {
      const response = await fetch(
        `/api/getcomments?post_id=${postId}&limit=10&offset=${this.offset}`
      );
